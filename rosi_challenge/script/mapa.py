...
def VELODYNE(velodyne_data):
	...
	if(synchronizer>=50):
		...
		for i in range(0, len(velodyne_data.data), 12):
			...
			if( ( (z+0.05)>0) and (x>1)):
				if( (int(y*100+1500)<3000) and (int(x*100+5000)<10000) ):
				...
				if( (int(y_global+1500)<3000) and (int(x_global+6000)<7000) ):
		...
		mapa = cv2.dilate(mapa, cv2.getStructuringElement(cv2.MORPH_RECT, (5, 5)), iterations=3)
...
def listener():
	...
	rospy.Subscriber('/sensor/velodyne', PointCloud2, VELODYNE,  buff_size=52428800)
	rospy.spin()

...
def orientation(data):						# reads robot's orientation published by the IMU
	...
	pointer *= orientation											# pointer state definition relative to robot's orientation
	
	firstTerm = 2 * ((X*Y) + (Z*W))										
	secondTerm = (1 - (2*((Z*Z) + (W*W))))								
	yawCorrection = -np.arctan(firstTerm/secondTerm) * orientation	
